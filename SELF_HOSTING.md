...
- 默认启用生产模式，没有固定验证码。建议在 `.env.codeloom` 配置内部 SMTP；
  ...
  必须保护日志访问。
  Web 首次登录不再收集姓名、角色、用途或来源问卷，也不强制安装运行时：
  已有工作区直接复用；没有工作区时自动创建名为 `Codeloom` 的工作区，
  以 `codeloom-<用户 UUID>` 为唯一 slug，然后进入任务主页。
  不自动加入其他用户的工作区；团队邀请仍需确认。工作区名称与个人资料可在设置中修改。
  刷新或多个标签页同时初始化不会重复创建工作区；初始化失败显示错误并可重试。
...
首次登录简化另经独立数据库与真实浏览器验证：空账号自动进入中文任务主页，
已有工作区复用、刷新后保持主页；并发创建请求得到 `201` / `409` 后仍只有一个工作区，
问卷保持 `{}`，未创建教程任务或 Agent。登录回调与工作区布局 25 个回归测试、
前端类型检查及相关 ESLint 检查通过。
